...
def main():
	...
	for log_file in log_files:
		...
		with open(RESULTS_FOLDER + log_file, 'rb') as f:
			...
			else: # log files starts with log_sim_rl, so go through this way
				for line in f: # corresponding log files in results directory
					parse = line.split() # split each line with spaceholder
					if len(parse) <= 1:
						break
		...
		for scheme in SCHEMES:
			if scheme in log_file:
				# schme:='sim_rl', behand is log_file name, (total 127)
	...
	for scheme in SCHEMES:
		ax.plot(reward_all[scheme]) # ploting
	...
	ax.legend(SCHEMES_REW, loc=4) # add a title lie in sim_rl
